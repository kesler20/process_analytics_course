...
#Do every 10 for the validation to speed things up
nir_spectra_2_use_val = nir_spectra_2_use[1:nir_spectra_2_use.shape[0]:10,:]
Ck_val                = Ck[1:Ck.shape[0]:10,:]
dose_source_val       = dose_source[1:dose_source.shape[0]:10,:]
...
# Build  Unsupervised EIOT Model with 1 interference and plot
eiot_obj = eiot.build(nir_spectra_2_use_cal,Ck_cal,num_si_u=2)
plt.figure()
plt.plot(eiot_obj['S_hat'].T)
plt.title("S'_hat EIOT")

plt.figure()
plt.plot(eiot_obj['S_I'].T)
plt.title("SI_1 EIOT")

#%%Are there interferences of known origin?

pls_obj=phi.pls(dose_source_cal,nir_spectra_2_use_cal,3)
pp.r2pv(pls_obj)

pls_obj=phi.pls(dose_source_cal[:,[0,2]],nir_spectra_2_use_cal,3)
pp.r2pv(pls_obj)

dose_source_cal=dose_source_cal[:,[0,2]]
dose_source_val=dose_source_val[:,[0,2]]
#%%
#Now build a supervised model and show lambas and estimated interferences
...
plt.figure()
plt.plot(eiot_obj_S['S_hat'].T)
plt.title("S'_hat SEIOT")

plt.figure()
plt.plot(eiot_obj_S['S_I'].T)
plt.title("SI SEIOT")

#%%
# Build  Supervised EIOT Model and plot lambdas
# This should take like 2 min to complete
#PLOT RMSE vs # of NCI
rmse_vs_nci_ps = []
rmse_vs_nci_as = []
for nci in [0,1,2,3,4,5]:
...
fig,ax=plt.subplots()
ax.plot(list(range(0,len(rmse_vs_nci_ps))),rmse_vs_nci_ps,'ob',label='Passive Supervision')
ax.plot(list(range(0,len(rmse_vs_nci_as))),rmse_vs_nci_as,'or',label='Active Supervision')
...
#Compare the active vs passive supervision of the interference
rik_hat=(pred_sup_ps['r_I_hat'][:,[0,1]]*np.tile(eiot_obj_S[ 'Rk_std'],(pred_sup_ps['r_I_hat'].shape[0],1))
         +np.tile(eiot_obj_S[ 'Rk_mean'],(pred_sup_ps['r_I_hat'].shape[0],1)))
plt.figure()
plt.plot(dose_source_val)
plt.plot(rik_hat,'.')
plt.title('Known interferences real vs estimated')
